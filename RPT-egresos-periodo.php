...
$pdf->Cell(0 ,10,'LISTADO DE EGRESOS PERIODO '.$desc_per.'',0, 1 , 'C');
$pdf->Ln();
//$pdf->Cell(10 ,8,'Nro.',1, 0 , 'C');
$pdf->SetFont('Arial','I',10);
$pdf->Cell(25 ,8,'Fecha',1, 0 , 'C');
$pdf->Cell(105 ,8,'Descripcion',1, 0 , 'C');
...
//CONSULTA EGRESOS
$consulta2= "SELECT * FROM egreso WHERE month (fecha_egreso)=$mes AND year (fecha_egreso)=$anio AND consorcio_idconsorcio=$id_consorcio  ORDER BY fecha_egreso";
...
while ($fila2 = mysqli_fetch_assoc($resultado2))
    {
        ...
        $fecha          = $fila2 ['fecha_egreso'];
        $monto          = $fila2 ['monto_egreso']; 
        $descripcion_eg = $fila2 ['descripcion_egreso'];
        $tipo           = $fila2 ['tipo_egreso'];
        ...
        else
        {
            ...
            $pdf->Cell(25 ,8,''.$fecha.'',1, 0 , 'C');
            $pdf->Cell(105 ,8,''.$descripcion_eg.'',1, 0 , 'C');
            $pdf->Cell(25 ,8,'$ '.$monto.'',1, 0 , 'C');
            $pdf->Cell(15 ,8,''.$tipo.'',1,0, 'C');
