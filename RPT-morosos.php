...
$pdf->Cell(0 ,10,'LISTADO DE MOROSOS ',0, 1 , 'C');
$pdf->Ln();
...
$consulta2= "SELECT * FROM liquidacion INNER JOIN liquidacion_unidad INNER JOIN unidad INNER JOIN pago_liquidacion INNER JOIN periodo ON periodo_idperiodo=idperiodo AND idliquidacion=liquidacion_idliquidacion AND consorcio_idconsorcio=$id_consorcio  AND unidad_idunidad=idunidad AND idliquidacion_unidad= liquidacion_unidad_idliquidacion_unidad AND estado =0  ORDER BY piso , departamento, anio, mes";
$resultado2=$connect->query($consulta2);
date_default_timezone_set("America/Argentina/Buenos_Aires");
$dma=date("Y/m/d");
$c=1;
 while($fila_t=mysqli_fetch_assoc($resultado2))
